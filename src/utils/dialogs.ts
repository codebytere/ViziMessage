...
export async function runContactsFailureDialog() {
  const result = await dialog.showMessageBox({
    ...
    defaultId: 0,
    detail: 'In order to use this application you need to give it access to Contacts. Open System Preferences?',
    message: 'Access to Contacts was not authorized.',
  });

  if (result.response === 0) {
    const fdaPath = 'x-apple.systempreferences:com.apple.preference.security?Privacy_Contacts';
...
export async function runFDAFailureDialog() {
  ...
  if (result.response === 0) {
    const fdaPath = 'x-apple.systempreferences:com.apple.preference.security?Privacy_AllFiles';
    await shell.openExternal(fdaPath);
  }
}
